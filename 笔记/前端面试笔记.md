...
46.bfc:浮动元素和绝对定位元素，非块级盒子的块级容器（例如 inline-blocks, table-cells, 和 table-captions），以及overflow值不为"visiable"的块级盒子，都会为他们的内容创建新的BFC（Block Fromatting Context， 即块级格式上下文）。
...
boolean,undefined,number,BigInt,string,Symbol,null
...
Map是字典，key值可以是任意类型，对象的key值只能是字符串

Set是集合，类似数组，但是里面的值具有唯一性，键和值相等
...
构造函数的prototype属性指向调用这个构造函数创建的实例的原型对象，实例对象的__proto__   属性指向它的原型对象，原型的constructor属性指向关联的构造函数
